...
class AuthProxy:
    ...
    def cleanup_proxy_authorizations(self):
        """清理proxy_authorizations字典以防止内存泄漏"""
        if len(self.proxy_authorizations) > MAX_PROXY_AUTH_SIZE:
            ...
            ctx.log.info(f"Cleaned proxy_authorizations: {len(items)} -> {len(self.proxy_authorizations)} entries")
    ...
    def http_connect(self, flow: http.HTTPFlow):
        ...
        # 定期清理内存以防止泄漏
        self.cleanup_proxy_authorizations()
    ...
    # Generated by Copilot - Optimized for better performance
    async def split_jsons(self, json_string):
        """优化的JSON分割方法，提升性能"""
        json_objects = []
        
        # 使用正则表达式预处理，找到潜在的JSON对象边界
        import re
        
        # 简单优化：按换行符分割，然后寻找完整的JSON
        # 这比逐字符遍历更高效
        potential_jsons = []
        current_json = ""
        brace_count = 0
            ...
            for char in line:
                if char == '{':
                    ...
                    if brace_count == 0 and current_json.strip():
                        try:
                            # 找到第一个{和最后一个}之间的内容
                            start_idx = current_json.find('{')
                            end_idx = current_json.rfind('}') + 1
                            if start_idx != -1 and end_idx > start_idx:
                                json_content = current_json[start_idx:end_idx]
                    ...
                    for obj in json_objects:
                        ...
                        if "hown" in baseDataName or "accepted" in baseDataName:
                            ...
                            try:
                                ...
                                ctx.log.error(f"Failed to save telemetry data to Elasticsearch: {e}")
                except Exception as e:
                    ctx.log.error(f"Failed to process telemetry data: {e}")
