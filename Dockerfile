...
# 安装任何额外的依赖项（如果需要）
RUN pip install mitmproxy elasticsearch redis chardet
...
CMD ["mitmdump", "--set", "confdir=/opt/mitmproxy", "-s", "proxy-es.py", "-p", "8080", "--listen-host", "0.0.0.0", "--set", "block_global=false", "--allow-hosts", "(.*copilot-proxy\\.githubusercontent\\.com.*|.*api\\.business\\.githubcopilot\\.com.*)"]
